...
function AccountScreen({ isLoaded, isSignedIn, user, profile, orders, onAuth, onSignOut, isSigningOut, onNavigate, onContact }: { isLoaded: boolean; isSignedIn: boolean; user: ReturnType<typeof useUser>["user"]; profile: Doc<"profiles"> | null | undefined; orders: Array<{ id: string; orderNumber: string; itemName: string; itemCount: number; amountCents: number; status: string; createdAt: number }> | undefined; onAuth: (mode: AuthMode) => void; onSignOut: () => Promise<void>; isSigningOut: boolean; onNavigate: (screen: Screen) => void; onContact: () => void }) {
...
export function MobileApp() {
  ...
  async function handleSignOut() {
    ...
    try {
      ...
      setActionError(reason instanceof Error ? reason.message : "We couldn’t sign you out. Please try again.");
    } finally {
